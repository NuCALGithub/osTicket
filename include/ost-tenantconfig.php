...
fwrite($file,json_encode($url).PHP_EOL);

$conn = new mysqli(DBHOST,DBUSER,DBPASS,DBSITES);
$sql = "select * from multitenant_tenant where url = '$url' and active = 1";
#$sql = "select * from multitenant where url = '$url'";
$res = $conn->query($sql);
$row = $res->fetch_array();
